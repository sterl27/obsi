...
__export(main_exports, {
  default: () => StellarGraphPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian = require("obsidian");
var VIEW_TYPE = "stellar-graph-view";
...
var StellarGraphView = class extends import_obsidian.ItemView {
  plugin;
  canvas;
  ctx;
  nodes = [];
  ...
  buildHud(stage) {
    ...
    this.iconButton(rail, "locate-fixed", "Focus neighbors", () => {
      this.focusDepth = (this.focusDepth + 1) % 3;
      this.updateFilter();
    });
    this.iconButton(rail, "route", "Find path from selected note", () => {
      this.path.clear();
      if (this.selected) {
        this.pathStart = this.selected;
        new import_obsidian.Notice("Select a destination note");
      } else new import_obsidian.Notice("Select a starting note first");
    });
    ...
    legend.createEl("div", { cls: "stellar-hint", text: "Drag to orbit \xB7 Scroll to zoom \xB7 Double-click to open" });
  ...
  async rebuild() {
    ...
    this.nodes = files.map((file, index) => {
      ...
      const node = { id: file.path, file, label: file.basename, group: this.plugin.settings.colorMode === "tag" ? tags[0] ?? "untagged" : folder, tags, degree: 0, x: Math.cos(angle) * radius, y: Math.sin(angle) * radius, vx: 0, vy: 0, pinned: false };
    ...
    for (const source of this.nodes) {
      ...
      for (const targetPath of Object.keys(links)) {
        ...
        const key = [source.id, target.id].sort().join("\0");
  ...
  neighborhood(start, depth) {
    const found = /* @__PURE__ */ new Set([start.id]);
    let frontier = [start];
    for (let i = 0; i < depth; i++) {
      ...
      for (const node of frontier) for (const edge of this.edges) {
        ...
        if (neighbor && !found.has(neighbor.id)) {
  ...
  tick(time) {
    ...
    this.lastFrame = time;
    if (this.plugin.settings.motion && this.temperature > 3e-3) this.simulate(delta);
    this.draw(time);
  ...
  simulate(delta) {
    ...
    for (let i = 0; i < active.length; i++) {
      const a = active[i];
      if (!a.pinned) {
        a.vx += -a.x * 22e-5 * delta;
        a.vy += -a.y * 22e-5 * delta;
      }
    ...
    for (const edge of this.edges) {
      ...
      const force = (distance - 90) * 18e-4 * this.temperature * delta;
  ...
  drawNode(node, time) {
    ...
    const onPath = this.path.has(node.id);
    ...
    if (selected || hovered || onPath) {
      ...
      this.ctx.beginPath();
      this.ctx.arc(node.x, node.y, radius * 2.7 * pulse, 0, Math.PI * 2);
      const glow = this.ctx.createRadialGradient(node.x, node.y, radius, node.x, node.y, radius * 2.8);
      glow.addColorStop(0, color.replace("1)", ".38)"));
    ...
    this.ctx.fillStyle = selected ? "#f8fbff" : color;
    this.ctx.fill();
  ...
  bindEvents() {
    ...
    this.canvas.addEventListener("pointermove", (event) => {
      ...
      if (this.dragNode) {
        ...
        this.temperature = Math.max(this.temperature, 0.18);
    ...
    this.canvas.addEventListener("dblclick", (event) => {
      const node = this.hitTest(event.offsetX, event.offsetY);
      if (node) void this.app.workspace.getLeaf(false).openFile(node.file);
    });
    this.canvas.addEventListener("wheel", (event) => {
      event.preventDefault();
      const factor = Math.exp(-event.deltaY * 12e-4);
  ...
  selectNode(node, center) {
    if (this.pathStart && this.pathStart !== node) {
      this.path = this.shortestPath(this.pathStart, node);
      this.pathStart = null;
    } else if (this.pathStart === null && this.path.size === 0) {
    }
    this.selected = node;
    if (center) {
      this.transform.x = -node.x * this.transform.scale;
      this.transform.y = -node.y * this.transform.scale;
    }
    this.updateFilter();
    this.renderInspector(node);
  }
  renderInspector(node) {
    const panel = this.contentEl.querySelector(".stellar-inspector");
    if (!panel) return;
    panel.empty();
    ...
    path.addEventListener("click", () => {
      this.pathStart = node;
      this.path.clear();
      new import_obsidian.Notice("Now select a destination note");
    });
  }
  shortestPath(start, end) {
    const queue = [start];
    const previous = /* @__PURE__ */ new Map([[start.id, null]]);
    while (queue.length) {
      const current = queue.shift();
      if (current === end) break;
      for (const edge of this.edges) {
        const next = edge.source === current ? edge.target : edge.target === current ? edge.source : null;
        if (next && !previous.has(next.id)) {
          previous.set(next.id, current.id);
          queue.push(next);
        }
      }
    }
    if (!previous.has(end.id)) {
      new import_obsidian.Notice("No link path found");
...
var StellarSettingTab = class extends import_obsidian.PluginSettingTab {
  plugin;
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Node scale").setDesc("Scale stars and hubs.").addSlider((slider) => slider.setLimits(0.6, 2, 0.1).setValue(this.plugin.settings.nodeScale).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.nodeScale = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Link opacity").setDesc("Control the visual weight of connections.").addSlider((slider) => slider.setLimits(0.05, 0.8, 0.05).setValue(this.plugin.settings.linkOpacity).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.linkOpacity = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Living layout").setDesc("Animate the force-directed layout as the graph settles.").addToggle((toggle) => toggle.setValue(this.plugin.settings.motion).onChange(async (value) => {
      this.plugin.settings.motion = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Show orphan notes").setDesc("Include notes with no resolved links.").addToggle((toggle) => toggle.setValue(this.plugin.settings.showOrphans).onChange(async (value) => {
      this.plugin.settings.showOrphans = value;
      await this.plugin.saveSettings();
      this.plugin.refreshViews();
    }));
    new import_obsidian.Setting(containerEl).setName("Cluster colors").setDesc("Color stars by their folder or first tag.").addDropdown((dropdown) => dropdown.addOption("folder", "Folder").addOption("tag", "First tag").setValue(this.plugin.settings.colorMode).onChange(async (value) => {
...
var StellarGraphPlugin = class extends import_obsidian.Plugin {
  ...
  refreshViews(rebuild = false) {
    for (const leaf of this.app.workspace.getLeavesOfType(VIEW_TYPE)) {
      const view = leaf.view;
      if (view instanceof StellarGraphView) {
        if (rebuild) void view.rebuild();
        else view.updateFilter();
      }
    }
  }
  async loadSettings() {
